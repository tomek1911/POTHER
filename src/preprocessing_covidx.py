...
##COVIDx
dataset_df = pd.read_csv(f'../CovidXBenchmark/benchmark_data/covidx_mask_{target}.csv')
# dataset_df = pd.read_csv(f'../CovidXBenchmark/benchmark_data/test_covidx.csv')
mask_paths = [f'../CovidXBenchmark/benchmark_data/{target}_masks_filtered_interpolated/' + filename for filename in dataset_df['filename'].tolist()]
# mask_paths = [f'../CovidXBenchmark/benchmark_data/{target}_masks_all/' + filename for filename in dataset_df['filename'].tolist()]

# Oh - Wang benchmark
# dataset_df = pd.read_csv(f'../CovidXBenchmark/wang_benchmark/wang_complete_no_rejected.csv')
# mask_paths = [f'../CovidXBenchmark/wang_benchmark/masks_interpolated/' + filename for filename in dataset_df['filename'].tolist()]
# image_paths = ['../CovidXBenchmark/benchmark_data/train_1024/' + filename for filename in dataset_df['filename'].tolist()]
...
def save_resized_images_and_masks():
    # folder = '../CovidXBenchmark/wang_benchmark/'
    folder = '../CovidXBenchmark/benchmark_data/'
    ...
    for i in range(len(mask_paths)):
        ...
        mask = cv2.rectangle(mask,(0,0),(1023,1023), 0, thickness=2)
        ...
        contours = [cnt for cnt in contours if len(cnt) > 600]
        
        # two lungs case
        if len(contours)==2:
            for cnt in contours:
                
                cnt = approximate_contour(cnt, eps=0.0025)
                cnt_scaled = scale_cnt(cnt, scale=scales[0])
                ### larger contour with white
                cv2.drawContours(
                    scaled_mask, [cnt_scaled], -1, color=(255, 255, 255), thickness=cv2.FILLED)
                ### smaller contour with black - to create whole inside
        ...
        cv2.imwrite(os.path.join(folder, f'{target}_masks_reduced9', mask_paths[i].split('/')[-1]), scaled_mask)
        # cv2.imwrite(os.path.join(folder, 'masks_draw', mask_paths[i].split('/')[-1]), scaled_mask)

        #save progress
        print(f"\r{i+1}/{len(mask_paths)}_{mask_paths[i].split('/')[-1]}", end="")
